...
export default function Chat({ activePage, setActivePage, user }) {
  ...
  useEffect(() => {
    let mounted = true
    getChatsAPI()
      .then(data => { if (mounted && data.success) setConversations(data.chats) })
      .catch(() => {})
  ...
  const handleDeleteChat = async (id) => {
    try {
      ...
      console.log('Could not delete chat')
    }
  }
  ...
  const exportAsWord = async () => {
    ...
    try {
      ...
      const docChildren = [
        ...
        new Paragraph({
          children: [new TextRun({ text: `Exported by: ${user?.name} | ${new Date().toLocaleString()}`, color: '888888', size: 20 })]
        }),
        new Paragraph({ text: '' }),
      ]
      for (const msg of chatMessages) {
        docChildren.push(
          new Paragraph({
            children: [new TextRun({ text: msg.role === 'user' ? `${user?.name}:` : 'NEURALIQ AI:', bold: true, color: msg.role === 'user' ? '7c3aed' : '06b6d4', size: 22 })]
          }),
            ...
            <div
              ...
              <div className="absolute right-2 top-2.5 flex gap-1 opacity-0 group-hover:opacity-100 transition-all">
                <button
                  ...
                  className="w-6 h-6 rounded flex items-center justify-center text-gray-500 hover:text-purple-400 hover:bg-white/10 transition-all text-xs"
                  title="Rename">
                  ✏️
                </button>
                <button
                  ...
                  className="w-6 h-6 rounded flex items-center justify-center text-gray-500 hover:text-red-400 hover:bg-white/10 transition-all text-xs"
                  title="Delete">
                  🗑️
                </button>
              </div>
